...
$progressPath = 'results\E2_5d_20280315_60s.progress.txt'
...
for ($s = 1; $s -le 30; $s++) {
    ...
    $output = (& java "-Dtasf.seed=$s" "-Dtasf.timeLimitMs=60000" -cp target/classes pe.edu.pucp.tasf.Main E2 5 20280315 0 2>&1) -join "`n"
    Set-Content -Path ("results\E2_5d_20280315_60s_seed_{0}.txt" -f $s) -Value $output -Encoding UTF8
    $row = [pscustomobject]@{
        Replica = $s
        Seed = $s
    ...
    $rows += $row
    $rows | Export-Csv -NoTypeInformation -Encoding UTF8 -Path results\E2_5d_20280315_60s_30_runs.partial.csv
    Add-Content -Path $progressPath -Value "done replica=$s seed=$s fitness=$($row.Fitness) time=$($row.ExecutionTime) end=$(Get-Date -Format o)"
}
$rows | Export-Csv -NoTypeInformation -Encoding UTF8 -Path results\E2_5d_20280315_60s_30_runs.csv
$md = @()
...
Set-Content -Path results\E2_5d_20280315_60s_30_runs.md -Value $md -Encoding UTF8
